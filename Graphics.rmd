...
- until now we only used the x and y coordinate as aestetics
- `?geom_point()` tells us about further aestetics that we can map data to 
- note that each geom has its own set of aestetics
...
## Statitic Transformations and Aestetics

- if we add a layer with a statistic transformation such as `geom_smooth()` 
  to a plot where we mapped one ore more aesthetics to discrete variables, then
  the transformation is performed by group 
...
## Statitic Transformations and Aestetics
- If this is not desired simply move color and shape as additional aestetics to `geom_point()`
```{r, echo = TRUE, eval = FALSE}
ggplot(diamonds, aes(x = carat, y = price)) + 
...
- in some circumstances it might also be convinient to leave all aestetics in `ggplot()` (e.g. if there are many layers) and overwrite the created groups by a arbitrary constant value
...
## Statistic Transformations and Aestetics
```{r, echo = FALSE, eval = TRUE}
ggplot(diamonds, aes(x = carat, y = price, color = color, shape = cut)) + 
  geom_point() +
