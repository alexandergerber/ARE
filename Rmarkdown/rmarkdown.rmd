...
- R Markdown is build on [Pandoc Markdown](https://rmarkdown.rstudio.com/authoring_pandoc_markdown.html%23raw-tex#pandoc_markdown), which in turn is a flavour of the markup language Markdown.  
...
If you use mainly chunk options which are not the default, then it saves you work to change the default for your document with ``knitr::opts_chunk$set()`.  
...
- If your code produces raw HTML or $\LaTeX{}$ for e.g. a table then you usually have to set in the  chunk options `results='asis'`, otherwise it gets formatted and will not work as intended
...
4. Optimize your code chunks so that the document doesn't everytime you compile loads the data. Also let us see what you did by making the code visible. 
