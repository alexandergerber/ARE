...
- to generate reproducible reports, articles, books ...
- as an environment for data science, as a lab notebook where you can capture not only what you did but also what you were thinking. 
...
If this is stored as `example.rmd` in your working directory, running `knitr::render("example.rmd")` will outut an html document. Alternatively, hit the `r knitr::include_graphics("../img/knit.PNG")` button in RStudio. 
---
class: left, top
...
An R Markdown document basically is a text file with extension `.Rmd`.

The example already contains the most important components of an `.Rmd` file:
...
What happens when we click **knit** in RStudio? 
...
A Basic YAML header using mostly defaults:
```yaml
---
title: Habits
author: John Doe
...
Note that Markdown does not support nested ordered lists with numbering 1.1, 1.2.1 and so on. For this, you would have to step up to HTML or $\LaTeX$. 
...
When rendering to PDF, $\LaTeX{}$ can be used throughout the document. `MathJax` allows using $\LaTeX{}$ math mode for HTML output as follows: 
...
If a chunk options is frequently used it may save time to change the default for the document using  `knitr::opts_chunk$set()`.  
...
You can also use R inline. This is handy because it ensures that computed results which you refer to adjust if you update your code. &rarr; Less error prone and less time consuming than manual typing.
...
<br> <br>
In general, however, tables should be produced by code. The following slides show some examples. 
...
- If the content of an `.Rmd` is only Markdown the document can be compiled to all supported output formats without trouble. 

- However, R Markdown sometimes is too limited and we would like to use more complex functions or typesetting as available in HTML or $\LaTeX{}$ libraries. 

  - If the output format is HTML you can also use HTML instead of R Markdown 
  or mix both.

  - If the output format is PDF mixing $\LaTeX{}$ and HTML is fine.
...
- An interactive map. 
    ...
    ```
    ````
- If you want to learn more go to [Leaflet for R](https://rstudio.github.io/leaflet).
---
class: left, top
## Interactive documents
...
1. Create an R Markdown document that compiles to an HTML document. 
...
4. Optimize your code chunks so that the document doesn't load the data everytime you compile. Also let other see your approach by making the code visible. 

5. Create a PDF document containing the results.

6. Remember that nested ordered list are not supported by R Markdown. Nonetheless, generate a nested ordered list in both HTML and PDF output. 
