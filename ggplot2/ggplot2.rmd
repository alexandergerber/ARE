...
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "stack") # the default
    ```
- Instead of stacking the bars, we can put the bars for each group next to each other with `dodge`. 
    ```{r, echo = TRUE, eval = FALSE, fig.width=10}
  geom_bar(., position = "dodge") 
    ```
- We can compare relative proportions at each x by stacking the bars and  standardising each bar to have the same height with `fill`.
    ```{r, echo = TRUE, eval = FALSE, fig.width=10}
