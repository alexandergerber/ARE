...
1. 
    ...
    (b) Estimate $A$ using `A_loop(N)` with 500000 MC iterations.

2. 

    (a) Define a function `A_vec(N)` which implements the MC algorithm in a vectorized fashion.
    
    (b) Compare `A_vec(N)` and `A_loop(N)` in a microbenchmark for `N=500000` iterations.
...
Gillespie, C. and Lovelace, *R. Efficient R Programming*. O'Reilly Media.
