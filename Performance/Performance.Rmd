...
**Example: coercation to another type**
...
---
background-image: url(img/profiling_coercation.png)
...
Especially if a large number of computations need to be performed, coercation should be avoided wherever possible!
...
If the above are not enough, you may rewrite (some of) the code in C++ using Rcpp (discussed in the next chapter).
...
**Example: coercation of inputs**

`apply()` allows for various inputs and outputs which requires coercation.
...
Testing equality is *much* simpler than testing inclusion in a set.
...
The approach used in `a()` is what we teach undergraduates which is totally fine &mdash; except if you want (only) $SE(\widehat\beta)$ and fast.

The code in function `b()` is rather focussed on the essentials.
...
- is the process of converting an algorithm from operating on a single value at a time to operating on a set of values at one time.

- makes problems simpler: code that operates on vectors instead of single entries of a vector is often less complex to write.

- may avoid loops in R. Loops in vectorized functions are written in C and are much faster.

**Use Matrix Algebra**

Loops used in most matrix operations are executed by highly optimized external libraries. See, e.g., [BLAS](https://en.wikipedia.org/wiki/Basic_Linear_Algebra_Subprograms).

**Use vectorized functions in base R**

- `rowSums()`, `colSums()`, `rowMeans()`, `colMeans()`, `cumsum()`, `diff()`


---
background-image: url(img/atomic_vectors.png)
background-position: 50% 80%
...
---
background-image: url(img/rowcolumnarrays.jpg)
background-position: 50% 80%
...
R saves data in [column-major order](https://en.m.wikipedia.org/wiki/Row-_and_column-major_order). 

To disregard this pattern by writing code which operates on $1\times1$ vectors thus means  'fighting the language'.
