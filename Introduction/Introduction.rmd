...
- Over the semester there will be three graded assignments.  
...
- R is highly extensible. A massive set of packages for statistical modelling, machine learning, visualisation, and importing and manipulating data. Researchers in statistics and machine learning will often publish an R package to accompany their articles.
...
The main sources for packages are
  ...
  - GitHub (that is the place were packages are developed before getting published on CRAN) 
  - Bioconductor
...
- Use `devtools::install_github("tidymodels/broom")` to install the `broom` package from Github.
...
The probability is high that somebody else has already asked the same questions and it is already solved. 

If you don't find an answer you can ask a question yourself. What sounds easy is harder then on would think. Most importantly you have to provide a [reproducible example](https://stackoverflow.com/questions/5963269/how-to-make-a-great-r-reproducible-example).

---
## Rstudio
- RStudio is an integrated development environment (IDE) for R. 
- It provides some convenience functionality compared to the IDE shipping with R.
...
Workflow:
- Instead of openening a new R session and use `setwd()` you open the project. The working path is 
...
An in depth treatment of this material can be found in 
[Advanced R](https://adv-r.hadley.nz/vectors-chap.html#introduction-2) by Hadley Wickham. 
...
You should be familiar with the common subsetting operators  `[`, `[[` and `$`. 
...
- Lets make things more confusing. Can you predict the result?   
    ```{r}
x <- 3
...
What is the result of the first call two `square_4()`? What will happen the second time?
```{r}
x <- 2
square_4 <- function(){
  (x <- x^2)
